...
solve :: Solver
solve = \ case
    css -> case spanCount null $ map (elemIndices '#') css of
        (ui,_) -> trace ("ui = " ++ show ui) $ case drop ui css of
            dss -> case spanCount (not . null) $ elemIndices '#' <$> dss of
                (bi,_) -> trace ("bi = " ++ show bi) $ case transpose $ take bi dss of
                    tss    -> case spanCount null $ elemIndices '#' <$> tss of
                        (uj,_) -> trace ("uj = " ++ show uj) $ case drop uj tss of
                            uss -> case spanCount (not. null) $ map (elemIndices '#') uss of
                                (bj,_)  -> trace ("bj = " ++ show bj) $ case transpose $ take bj $ drop uj $ transpose $ take bi $ drop ui css of
                                    pss -> case concatMap (elemIndices '.') pss of
                                        [j] -> trace ("j = " ++ show j) $ case concatMap (elemIndices '.') (transpose pss) of
                                            [i] -> trace ("i = " ++ show i) [succ ui + i, succ uj + j]
                                            a   -> trace (show a) invalid
                                        a   -> trace (show a) invalid
