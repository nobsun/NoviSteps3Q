...
solve = \ case
    n -> case divMod n 2 of
        (_,1) -> []
        (m,0) -> sort $ parenseqIter m
        _     -> impossible

parenseqIter :: Int -> [String]
parenseqIter n = concatMap (prod . unzip) $ kkksso n [[("","")]]
    where
        prod (xss,yss) = [foldl (flip (:)) ys xs | xs <- xss, ys <- yss]

kkksso,kkksse :: Int -> [[(String,String)]] -> [[(String,String)]]
kkksso 0 xs  = xs
kkksso n xs  = kkksse (n-1) (zipWith (++) (map oc xs ++ [  ]) ([[]] ++ map co xs))

kkksse 0 xs  = xs
kkksse n xs  = kkksso (n-1) (zipWith (++) (map oc xs ++ [[]]) ([[]] ++ map co xs))

oc,co :: [(String,String)] -> [(String,String)]
oc = map (('(':) *** (')':))
co = map ((')':) *** ('(':))
