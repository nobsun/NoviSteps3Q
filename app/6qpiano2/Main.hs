...
solve :: Solver
solve = \ case
    (as,bs) -> bool "No" "Yes" $ iter False (sort as) (sort bs)
    where
        iter _ []     _      = False
        iter f (_:xs) []     = f || iter True xs []
        iter f (x:xs) (y:ys) = case compare x y of
            LT | f         -> True
               | otherwise -> iter True  xs (y:ys)
            _              -> iter False (x:xs) ys
