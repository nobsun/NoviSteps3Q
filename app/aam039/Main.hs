...
solve :: Solver
solve = \ case
    (n,lrxs) -> zipWith phi <*> tail $ take n $ scanl1 (+) $ elems $ accumArray (+) 0 (0,n) qqs where
        qqs = concatMap psi lrxs
        psi (l,r,x) = [(pred l,x),(r,-x)]
        phi x y = case compare x y of
            LT -> '<'
            EQ -> '='
            GT -> '>'
...
wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n,_]:lrxs -> case f (n, toTriple <$> lrxs) of
        r -> [[r]]
