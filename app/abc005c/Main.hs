...
solve :: Solver
solve = \ case
    (t,as,bs) -> bool "no" "yes" $ iter as bs where
        iter aas bbs = case bbs of
            []   -> True
            b:bs -> case aas of
                []   -> False
                a:as -> case compare a b of
                    LT | t < subtract a b -> iter as bbs
                       | otherwise        -> iter as bs
                    EQ                    -> iter as bs
                    GT                    -> False

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [t]:_:as:_:bs:_ -> case f (t,as,bs) of
        r -> [[r]]
