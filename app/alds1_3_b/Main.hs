...
solve :: Solver
solve = \ case
    (q,pps) -> iter 0 (Q.fromList pps) where
        iter pt qq = case Q.viewl qq of
            Q.EmptyL -> []
            (p,t) Q.:< rq
                | t > q -> iter (pt + q) (rq Q.|> (p, t - q))
                | otherwise -> (p, pt + t) : iter (pt + t) rq

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [_,q]:pps -> case f (read q, map (second read . toTuple) pps) of
        rr -> map (fromTuple . second show) rr
