...
solve :: Solver
solve = \ case
    as -> case foldl' phi M.empty as of
        mp -> case M.spanAntitone (50000 >) mp of
            (mp1,mp2) -> M.foldlWithKey' psi c0 mp1 where
                c0 = maybe 0 ((`div` 2) . ((*) <*> pred)) (mp2 M.!? 50000)
                psi a k v = a + v * fromMaybe 0 (mp2 M.!? subtract k 100000)
        where
            phi s k = M.insertWith (+) k 1 s
