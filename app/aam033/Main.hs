...
solve = \ case
    (p,a,b)
        | innerprod2d ab ap < 0 -> sqrt $ fromIntegral $ innerprod2d ap ap
        | innerprod2d ba bp < 0 -> sqrt $ fromIntegral $ innerprod2d bp bp
        | otherwise             -> fromIntegral (abs $ crossprod2d ap bp)
                                    / sqrt (fromIntegral $ innerprod2d ab ab) 
        where
            ab = sub2d a b
            ba = neg2d ab
            ap = sub2d a p
...
wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    pps -> case f (toTriple $ take 3 $ map toTuple pps) of
        r -> [[r]]
...
add2d (a,b) (c,d) = (a+c,b+d)
neg2d :: (Num a) => V2d a -> V2d a
neg2d (a,b) = (negate a, negate b)
sub2d :: (Num a) => V2d a -> V2d a -> V2d a
sub2d ab = add2d (neg2d ab)
crossprod2d :: (Num a) => V2d a -> V2d a -> a
crossprod2d (a,b) (c,d) = a*d - b*c
(×) :: (Num a) => V2d a -> V2d a -> a
(×) = crossprod2d

innerprod2d :: (Num a) => V2d a -> V2d a -> a
innerprod2d (a,b) (c,d) = a*c + b*d

direction2d ::  (Num a) => V2d a -> V2d a -> V2d a -> a
direction2d p q r = signum $ sub2d p q × sub2d p r

onsegment ::   (Num a, Ord a) => V2d a -> V2d a -> V2d a -> Bool
onsegment p q r = min p q <= r && r <= max p q
