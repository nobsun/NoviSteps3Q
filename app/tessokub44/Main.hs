...
solve :: Solver
solve = \ case
    (lss,qs) -> catMaybes $ snd $ mapAccumL phi (IM.fromList . zip [1 ..] $ IM.fromList . zip [1 ..] <$> lss) qs where
        phi m = \ case
            [1,x,y] -> (IM.insert x (m IM.! y) (IM.insert y (m IM.! x) m), Nothing)
            [2,x,y] -> (m, Just $ m IM.! x IM.! y)
            _       -> invalid

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n]:rss -> case f $ second tail $ splitAt n rss of
        rr -> (:[]) <$> rr
