...
solve :: Solver
solve = \ case
    (c,k,ts) -> case sort ts of
        us       -> iter 0 us where
            iter a = \ case
                []   -> a
                vs@(v:_) -> case span ((k >=) . subtract v) vs of
                    (ws,xs)
                        | length ws > c -> iter (succ a) (drop c ws ++ xs)
                        | otherwise     -> iter (succ a) xs
...
wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [_,c,k]:ts -> case f (c,k,concat ts) of
        r -> [[r]]
...
direction2d p q r = sub2d p q × sub2d p r
